...
namespace MixERP.Net.Core.Modules.HRM.Data
{
    ...
    /// </summary>
    public class PayGrade : DbAccess, IPayGradeRepository
    {
        /// <summary>
        ...
        /// </summary>
        public override string _ObjectName => "pay_grades";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"PayGrade\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "hrm.pay_grades" to return all instances of the "PayGrade" class. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.PayGrade> GetAll()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"PayGrade\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.pay_grades ORDER BY pay_grade_id;";
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql);
        }

        /// <summary>
        /// Executes a select query on the table "hrm.pay_grades" to return all instances of the "PayGrade" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "PayGrade" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        public MixERP.Net.Entities.HRM.PayGrade Get(int payGradeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"PayGrade\" filtered by \"PayGradeId\" with value {PayGradeId} was denied to the user with Login ID {_LoginId}", payGradeId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.pay_grades WHERE pay_grade_id=@0;";
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql, payGradeId).FirstOrDefault();
        }

        /// <summary>
        /// Gets the first record of the table "hrm.pay_grades". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "PayGrade" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.PayGrade GetFirst()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the first record of entity \"PayGrade\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.pay_grades ORDER BY pay_grade_id LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql).FirstOrDefault();
        }

        /// <summary>
        /// Gets the previous record of the table "hrm.pay_grades" sorted by payGradeId.
        /// </summary>
        ...
        public MixERP.Net.Entities.HRM.PayGrade GetPrevious(int payGradeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the previous entity of \"PayGrade\" by \"PayGradeId\" with value {PayGradeId} was denied to the user with Login ID {_LoginId}", payGradeId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.pay_grades WHERE pay_grade_id < @0 ORDER BY pay_grade_id DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql, payGradeId).FirstOrDefault();
        }

        /// <summary>
        /// Gets the next record of the table "hrm.pay_grades" sorted by payGradeId.
        /// </summary>
        /// <param name="payGradeId">The column "pay_grade_id" parameter used to find the next record.</param>
        /// <returns>Returns a non-live, non-mapped instance of "PayGrade" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.PayGrade GetNext(int payGradeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the next entity of \"PayGrade\" by \"PayGradeId\" with value {PayGradeId} was denied to the user with Login ID {_LoginId}", payGradeId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.pay_grades WHERE pay_grade_id > @0 ORDER BY pay_grade_id LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql, payGradeId).FirstOrDefault();
        }


        /// <summary>
        /// Gets the last record of the table "hrm.pay_grades". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "PayGrade" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.PayGrade GetLast()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the last record of entity \"PayGrade\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.pay_grades ORDER BY pay_grade_id DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.PayGrade> Get(int[] payGradeIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"PayGrade\" was denied to the user with Login ID {LoginId}. payGradeIds: {payGradeIds}.", this._LoginId, payGradeIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql, payGradeIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"PayGrade\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"PayGrade\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public object Add(dynamic payGrade)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"PayGrade\" was denied to the user with Login ID {LoginId}. {PayGrade}", this._LoginId, payGrade);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this._Catalog, payGrade, "hrm.pay_grades", "pay_grade_id");
        }
        ...
        public List<object> BulkImport(List<ExpandoObject> payGrades)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"PayGrade\" was denied to the user with Login ID {LoginId}. {payGrades}", this._LoginId, payGrades);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this._Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (dynamic payGrade in payGrades)
                        {
                            line++;
                            ...
                            {
                                result.Add(db.Insert("hrm.pay_grades", "pay_grade_id", payGrade));
                            }
                        }
        ...
        public void Update(dynamic payGrade, int payGradeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"PayGrade\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {PayGrade}", payGradeId, this._LoginId, payGrade);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this._Catalog, payGrade, payGradeId, "hrm.pay_grades", "pay_grade_id");
        }
        ...
        public void Delete(int payGradeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"PayGrade\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", payGradeId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM hrm.pay_grades WHERE pay_grade_id=@0;";
            Factory.NonQuery(this._Catalog, sql, payGradeId);
        }

        /// <summary>
        /// Performs a select statement on table "hrm.pay_grades" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.PayGrade> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"PayGrade\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.PayGrade> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"PayGrade\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.HRM.PayGrade>(this._Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"PayGrade\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "hrm.pay_grades" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.PayGrade> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"PayGrade\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"PayGrade\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.PayGrade> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"PayGrade\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
