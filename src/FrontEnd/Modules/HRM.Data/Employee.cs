...
namespace MixERP.Net.Core.Modules.HRM.Data
{
    ...
    /// </summary>
    public class Employee : DbAccess, IEmployeeRepository
    {
        /// <summary>
        ...
        /// <summary>
        /// Executes a select query on the table "hrm.employees" to return all instances of the "Employee" class. 
        /// </summary>
        ...
        /// <summary>
        /// Executes a select query on the table "hrm.employees" to return all instances of the "Employee" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "Employee" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        /// <summary>
        /// Gets the first record of the table "hrm.employees". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "Employee" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.Employee GetFirst()
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the first record of entity \"Employee\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.employees ORDER BY employee_id LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.Employee>(this._Catalog, sql).FirstOrDefault();
        }

        /// <summary>
        /// Gets the previous record of the table "hrm.employees" sorted by employeeId.
        /// </summary>
        /// <param name="employeeId">The column "employee_id" parameter used to find the next record.</param>
        /// <returns>Returns a non-live, non-mapped instance of "Employee" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.Employee GetPrevious(int employeeId)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the previous entity of \"Employee\" by \"EmployeeId\" with value {EmployeeId} was denied to the user with Login ID {_LoginId}", employeeId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.employees WHERE employee_id < @0 ORDER BY employee_id DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.Employee>(this._Catalog, sql, employeeId).FirstOrDefault();
        }

        /// <summary>
        /// Gets the next record of the table "hrm.employees" sorted by employeeId.
        /// </summary>
        /// <param name="employeeId">The column "employee_id" parameter used to find the next record.</param>
        /// <returns>Returns a non-live, non-mapped instance of "Employee" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.Employee GetNext(int employeeId)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the next entity of \"Employee\" by \"EmployeeId\" with value {EmployeeId} was denied to the user with Login ID {_LoginId}", employeeId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.employees WHERE employee_id > @0 ORDER BY employee_id LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.Employee>(this._Catalog, sql, employeeId).FirstOrDefault();
        }


        /// <summary>
        /// Gets the last record of the table "hrm.employees". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "Employee" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.HRM.Employee GetLast()
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the last record of entity \"Employee\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.employees ORDER BY employee_id DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.HRM.Employee>(this._Catalog, sql).FirstOrDefault();
        }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object AddOrEdit(dynamic employee, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
            ...
            employee.audit_user_id = this._UserId;
            employee.audit_ts = System.DateTime.UtcNow;

            object primaryKeyValue = employee.employee_id;

            if (Cast.To<int>(primaryKeyValue) > 0)
            {
                primaryKeyValue = employee.employee_id;
                this.Update(employee, int.Parse(employee.employee_id));
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(dynamic employee)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
            ...
            return Factory.Insert(this._Catalog, employee, "hrm.employees", "employee_id");
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<ExpandoObject> employees)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                using (Database db = new Database(Factory.GetConnectionString(this._Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (dynamic employee in employees)
                        {
                            ...
                            if (Cast.To<int>(primaryKeyValue) > 0)
                            {
                                result.Add(employee.employee_id);
                                db.Update("hrm.employees", "employee_id", employee, employee.employee_id);
                            }
                            else
                            {
                                result.Add(db.Insert("hrm.employees", "employee_id", employee));
                            }
                        }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void Update(dynamic employee, int employeeId)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return;
            ...
            Factory.Update(this._Catalog, employee, employeeId, "hrm.employees", "employee_id");
        }
