...
namespace MixERP.Net.Schemas.Office.Data
{
    ...
    public class GetSysUserIdProcedure : DbAccess, IGetSysUserIdRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "get_sys_user_id";
        /// <summary>
        ...
        public int Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"GetSysUserIdProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM office.get_sys_user_id();";
