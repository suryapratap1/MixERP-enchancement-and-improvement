...
namespace MixERP.Net.Schemas.Transactions.Data
{
    ...
    public class GetIncomeExpenditureStatementProcedure : DbAccess, IGetIncomeExpenditureStatementRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "get_income_expenditure_statement";
        /// <summary>
        ...
        public IEnumerable<DbGetIncomeExpenditureStatementResult> Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"GetIncomeExpenditureStatementProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM transactions.get_income_expenditure_statement(@DateFrom, @DateTo, @UserId, @OfficeId, @Compact);";
            ...
            return Factory.Get<DbGetIncomeExpenditureStatementResult>(this._Catalog, query, parameters.ToArray());
        }
