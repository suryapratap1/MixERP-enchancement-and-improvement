...
namespace MixERP.Net.Schemas.Transactions.Data
{
    ...
    /// </summary>
    public class PostStockAdjustmentProcedure : DbAccess, IPostStockAdjustmentRepository
    {
        /// <summary>
        ...
        /// Prepares, validates, and executes the function "transactions.post_stock_adjustment(_office_id integer, _user_id integer, _login_id bigint, _value_date date, _reference_number character varying, _statement_reference text, _details transactions.stock_adjustment_type[])" on the database.
        /// </summary>
        public PostStockAdjustmentProcedure()
        {
        }
        ...
        public long Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"PostStockAdjustmentProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM transactions.post_stock_adjustment(@OfficeId, @UserId, @LoginId, @ValueDate, @ReferenceNumber, @StatementReference, @Details);";
