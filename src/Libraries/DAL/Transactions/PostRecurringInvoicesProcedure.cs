...
namespace MixERP.Net.Schemas.Transactions.Data
{
    ...
    /// </summary>
    public class PostRecurringInvoicesProcedure : DbAccess, IPostRecurringInvoicesRepository
    {
        /// <summary>
        ...
        /// Prepares, validates, and executes the function "transactions.post_recurring_invoices(_user_id integer, _login_id bigint, _office_id integer, _value_date date)" on the database.
        /// </summary>
        public PostRecurringInvoicesProcedure()
        {
        }
        ...
        public void Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"PostRecurringInvoicesProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM transactions.post_recurring_invoices(@UserId, @LoginId, @OfficeId, @ValueDate);";
