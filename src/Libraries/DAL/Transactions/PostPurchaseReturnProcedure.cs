...
namespace MixERP.Net.Schemas.Transactions.Data
{
    ...
    public class PostPurchaseReturnProcedure : DbAccess, IPostPurchaseReturnRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "post_purchase_return";
        /// <summary>
        ...
        /// <param name="loginId">Enter argument value for "_login_id" parameter of the function "transactions.post_purchase_return".</param>
        ...
        public long Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"PostPurchaseReturnProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM transactions.post_purchase_return(@TransactionMasterId, @OfficeId, @UserId, @LoginId, @ValueDate, @StoreId, @PartyCode, @PriceTypeId, @ReferenceNumber, @StatementReference, @Details, @Attachments);";
