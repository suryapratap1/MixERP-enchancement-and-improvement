...
namespace MixERP.Net.Schemas.Transactions.Data
{
    ...
    /// </summary>
    public class DayOperation : DbAccess, IDayOperationRepository
    {
        /// <summary>
        ...
        /// </summary>
        public override string _ObjectName => "day_operation";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"DayOperation\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "transactions.day_operation" to return all instances of the "DayOperation" class. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.DayOperation> GetAll()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"DayOperation\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM transactions.day_operation ORDER BY day_id;";
            return Factory.Get<MixERP.Net.Entities.Transactions.DayOperation>(this._Catalog, sql);
        }

        /// <summary>
        /// Executes a select query on the table "transactions.day_operation" to return all instances of the "DayOperation" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "DayOperation" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        public MixERP.Net.Entities.Transactions.DayOperation Get(long dayId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"DayOperation\" filtered by \"DayId\" with value {DayId} was denied to the user with Login ID {_LoginId}", dayId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM transactions.day_operation WHERE day_id=@0;";
            return Factory.Get<MixERP.Net.Entities.Transactions.DayOperation>(this._Catalog, sql, dayId).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.DayOperation> Get(long[] dayIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"DayOperation\" was denied to the user with Login ID {LoginId}. dayIds: {dayIds}.", this._LoginId, dayIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Transactions.DayOperation>(this._Catalog, sql, dayIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"DayOperation\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"DayOperation\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public object AddOrEdit(dynamic dayOperation, List<EntityParser.CustomField> customFields)
        {
            ...
            object primaryKeyValue = dayOperation.day_id;

            if (Cast.To<long>(primaryKeyValue) > 0)
            {
                primaryKeyValue = dayOperation.day_id;
                this.Update(dayOperation, long.Parse(dayOperation.day_id));
            }
            else
            {
        ...
        public object Add(dynamic dayOperation)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"DayOperation\" was denied to the user with Login ID {LoginId}. {DayOperation}", this._LoginId, dayOperation);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this._Catalog, dayOperation, "transactions.day_operation", "day_id");
        }
        ...
        public List<object> BulkImport(List<ExpandoObject> dayOperations)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"DayOperation\" was denied to the user with Login ID {LoginId}. {dayOperations}", this._LoginId, dayOperations);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this._Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (dynamic dayOperation in dayOperations)
                        {
                            line++;
                            ...
                            if (Cast.To<long>(primaryKeyValue) > 0)
                            {
                                result.Add(dayOperation.day_id);
                                db.Update("transactions.day_operation", "day_id", dayOperation, dayOperation.day_id);
                            }
                            else
                            {
                                result.Add(db.Insert("transactions.day_operation", "day_id", dayOperation));
                            }
                        }
        ...
        public void Update(dynamic dayOperation, long dayId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"DayOperation\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {DayOperation}", dayId, this._LoginId, dayOperation);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this._Catalog, dayOperation, dayId, "transactions.day_operation", "day_id");
        }
        ...
        public void Delete(long dayId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"DayOperation\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", dayId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM transactions.day_operation WHERE day_id=@0;";
            Factory.NonQuery(this._Catalog, sql, dayId);
        }

        /// <summary>
        /// Performs a select statement on table "transactions.day_operation" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.DayOperation> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"DayOperation\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM transactions.day_operation ORDER BY day_id LIMIT 10 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Transactions.DayOperation>(this._Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.DayOperation> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"DayOperation\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Transactions.DayOperation>(this._Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"DayOperation\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "transactions.day_operation" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.DayOperation> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"DayOperation\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"DayOperation\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.DayOperation> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"DayOperation\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
