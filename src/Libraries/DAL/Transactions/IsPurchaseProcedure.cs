...
namespace MixERP.Net.Schemas.Transactions.Data
{
    ...
    public class IsPurchaseProcedure : DbAccess, IIsPurchaseRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "is_purchase";
        /// <summary>
        ...
        public bool Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"IsPurchaseProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM transactions.is_purchase(@TransactionMasterId);";
