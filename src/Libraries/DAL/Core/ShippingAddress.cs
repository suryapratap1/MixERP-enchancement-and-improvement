...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    /// </summary>
    public class ShippingAddress : DbAccess, IShippingAddressRepository
    {
        /// <summary>
        ...
        /// </summary>
        public override string _ObjectName => "shipping_addresses";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "core.shipping_addresses" to return all instances of the "ShippingAddress" class. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "ShippingAddress" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> GetAll()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.shipping_addresses ORDER BY shipping_address_id;";
            return Factory.Get<MixERP.Net.Entities.Core.ShippingAddress>(this._Catalog, sql);
        }

        /// <summary>
        /// Executes a select query on the table "core.shipping_addresses" to return all instances of the "ShippingAddress" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "ShippingAddress" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        public MixERP.Net.Entities.Core.ShippingAddress Get(long shippingAddressId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"ShippingAddress\" filtered by \"ShippingAddressId\" with value {ShippingAddressId} was denied to the user with Login ID {_LoginId}", shippingAddressId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <returns>Returns a non-live, non-mapped collection of "ShippingAddress" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> Get(long[] shippingAddressIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. shippingAddressIds: {shippingAddressIds}.", this._LoginId, shippingAddressIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.shipping_addresses WHERE shipping_address_id IN (@0);";

            return Factory.Get<MixERP.Net.Entities.Core.ShippingAddress>(this._Catalog, sql, shippingAddressIds);
        }

        /// <summary>
        /// Custom fields are user defined form elements for core.shipping_addresses.
        /// </summary>
        /// <returns>Returns an enumerable custom field collection for the table core.shipping_addresses</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            {
                sql = "SELECT * FROM core.custom_field_definition_view WHERE table_name='core.shipping_addresses' ORDER BY field_order;";
                return Factory.Get<PetaPoco.CustomField>(this._Catalog, sql);
            }

            sql = "SELECT * from core.get_custom_field_definition('core.shipping_addresses'::text, @0::text) ORDER BY field_order;";
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Inserts or updates the instance of ShippingAddress class on the database table "core.shipping_addresses".
        /// </summary>
        ...
        public object AddOrEdit(dynamic shippingAddress, List<EntityParser.CustomField> customFields)
        {
            ...
            {
                primaryKeyValue = this.Add(shippingAddress);
            }

            string sql = "DELETE FROM core.custom_fields WHERE custom_field_setup_id IN(" +
                         "SELECT custom_field_setup_id " +
                         "FROM core.custom_field_setup " +
                         "WHERE form_name=core.get_custom_field_form_name('core.shipping_addresses')" +
                         ");";
            ...
            foreach (var field in customFields)
            {
                sql = "INSERT INTO core.custom_fields(custom_field_setup_id, resource_id, value) " +
                      "SELECT core.get_custom_field_setup_id_by_table_name('core.shipping_addresses', @0::character varying(100)), " +
                      "@1, @2;";
        ...
        public object Add(dynamic shippingAddress)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. {ShippingAddress}", this._LoginId, shippingAddress);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this._Catalog, shippingAddress, "core.shipping_addresses", "shipping_address_id");
        }

        /// <summary>
        /// Inserts or updates multiple instances of ShippingAddress class on the database table "core.shipping_addresses";
        /// </summary>
        ...
        public List<object> BulkImport(List<ExpandoObject> shippingAddresses)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. {shippingAddresses}", this._LoginId, shippingAddresses);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this._Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (dynamic shippingAddress in shippingAddresses)
                        {
                            line++;
                            ...
                            {
                                result.Add(db.Insert("core.shipping_addresses", "shipping_address_id", shippingAddress));
                            }
                        }
        ...
        public void Update(dynamic shippingAddress, long shippingAddressId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"ShippingAddress\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {ShippingAddress}", shippingAddressId, this._LoginId, shippingAddress);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this._Catalog, shippingAddress, shippingAddressId, "core.shipping_addresses", "shipping_address_id");
        }
        ...
        public void Delete(long shippingAddressId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"ShippingAddress\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", shippingAddressId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.shipping_addresses WHERE shipping_address_id=@0;";
            Factory.NonQuery(this._Catalog, sql, shippingAddressId);
        }

        /// <summary>
        /// Performs a select statement on table "core.shipping_addresses" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.shipping_addresses ORDER BY shipping_address_id LIMIT 10 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.ShippingAddress>(this._Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.ShippingAddress>(this._Catalog, sql, offset);
        }

        public List<EntityParser.Filter> GetFilters(string catalog, string filterName)
        {
            const string sql = "SELECT * FROM core.filters WHERE object_name='core.shipping_addresses' AND lower(filter_name)=lower(@0);";
            return Factory.Get<EntityParser.Filter>(catalog, sql, filterName).ToList();
        }

        /// <summary>
        /// Performs a filtered count on table "core.shipping_addresses".
        /// </summary>
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Sql sql = Sql.Builder.Append("SELECT COUNT(*) FROM core.shipping_addresses WHERE 1 = 1");
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.shipping_addresses" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
