...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    /// </summary>
    public class BonusSlabDetail : DbAccess, IBonusSlabDetailRepository
    {
        /// <summary>
        ...
        /// </summary>
        public override string _ObjectName => "bonus_slab_details";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "core.bonus_slab_details" to return all instances of the "BonusSlabDetail" class. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BonusSlabDetail> GetAll()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.bonus_slab_details ORDER BY bonus_slab_detail_id;";
            return Factory.Get<MixERP.Net.Entities.Core.BonusSlabDetail>(this._Catalog, sql);
        }

        /// <summary>
        /// Executes a select query on the table "core.bonus_slab_details" to return all instances of the "BonusSlabDetail" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "BonusSlabDetail" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        public MixERP.Net.Entities.Core.BonusSlabDetail Get(int bonusSlabDetailId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"BonusSlabDetail\" filtered by \"BonusSlabDetailId\" with value {BonusSlabDetailId} was denied to the user with Login ID {_LoginId}", bonusSlabDetailId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BonusSlabDetail> Get(int[] bonusSlabDetailIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. bonusSlabDetailIds: {bonusSlabDetailIds}.", this._LoginId, bonusSlabDetailIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.BonusSlabDetail>(this._Catalog, sql, bonusSlabDetailIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public object AddOrEdit(dynamic bonusSlabDetail, List<EntityParser.CustomField> customFields)
        {
            ...
            bonusSlabDetail.audit_user_id = this._UserId;
            bonusSlabDetail.audit_ts = System.DateTime.UtcNow;

            object primaryKeyValue = bonusSlabDetail.bonus_slab_detail_id;

            if (Cast.To<int>(primaryKeyValue) > 0)
            {
                primaryKeyValue = bonusSlabDetail.bonus_slab_detail_id;
                this.Update(bonusSlabDetail, int.Parse(bonusSlabDetail.bonus_slab_detail_id));
            }
            else
            {
        ...
        public object Add(dynamic bonusSlabDetail)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. {BonusSlabDetail}", this._LoginId, bonusSlabDetail);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this._Catalog, bonusSlabDetail, "core.bonus_slab_details", "bonus_slab_detail_id");
        }
        ...
        public List<object> BulkImport(List<ExpandoObject> bonusSlabDetails)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. {bonusSlabDetails}", this._LoginId, bonusSlabDetails);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this._Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (dynamic bonusSlabDetail in bonusSlabDetails)
                        {
                            line++;
                            ...
                            if (Cast.To<int>(primaryKeyValue) > 0)
                            {
                                result.Add(bonusSlabDetail.bonus_slab_detail_id);
                                db.Update("core.bonus_slab_details", "bonus_slab_detail_id", bonusSlabDetail, bonusSlabDetail.bonus_slab_detail_id);
                            }
                            else
                            {
                                result.Add(db.Insert("core.bonus_slab_details", "bonus_slab_detail_id", bonusSlabDetail));
                            }
                        }
        ...
        public void Update(dynamic bonusSlabDetail, int bonusSlabDetailId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"BonusSlabDetail\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {BonusSlabDetail}", bonusSlabDetailId, this._LoginId, bonusSlabDetail);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this._Catalog, bonusSlabDetail, bonusSlabDetailId, "core.bonus_slab_details", "bonus_slab_detail_id");
        }
        ...
        public void Delete(int bonusSlabDetailId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"BonusSlabDetail\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", bonusSlabDetailId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.bonus_slab_details WHERE bonus_slab_detail_id=@0;";
            Factory.NonQuery(this._Catalog, sql, bonusSlabDetailId);
        }

        /// <summary>
        /// Performs a select statement on table "core.bonus_slab_details" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BonusSlabDetail> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.bonus_slab_details ORDER BY bonus_slab_detail_id LIMIT 10 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.BonusSlabDetail>(this._Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BonusSlabDetail> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.BonusSlabDetail>(this._Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.bonus_slab_details" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BonusSlabDetail> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BonusSlabDetail> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"BonusSlabDetail\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
