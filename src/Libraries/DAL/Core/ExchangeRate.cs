...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class ExchangeRate : DbAccess, IExchangeRateRepository
    {
        ...
        /// <summary>
        /// Gets the first record of the table "core.exchange_rates". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "ExchangeRate" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.Core.ExchangeRate GetFirst()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the first record of entity \"ExchangeRate\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.exchange_rates ORDER BY exchange_rate_id LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Core.ExchangeRate>(this._Catalog, sql).FirstOrDefault();
        }

        /// <summary>
        /// Gets the previous record of the table "core.exchange_rates" sorted by exchangeRateId.
        /// </summary>
        ...
        public MixERP.Net.Entities.Core.ExchangeRate GetPrevious(long exchangeRateId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the previous entity of \"ExchangeRate\" by \"ExchangeRateId\" with value {ExchangeRateId} was denied to the user with Login ID {_LoginId}", exchangeRateId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.exchange_rates WHERE exchange_rate_id < @0 ORDER BY exchange_rate_id DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Core.ExchangeRate>(this._Catalog, sql, exchangeRateId).FirstOrDefault();
        }

        /// <summary>
        /// Gets the next record of the table "core.exchange_rates" sorted by exchangeRateId.
        /// </summary>
        /// <param name="exchangeRateId">The column "exchange_rate_id" parameter used to find the next record.</param>
        /// <returns>Returns a non-live, non-mapped instance of "ExchangeRate" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.Core.ExchangeRate GetNext(long exchangeRateId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the next entity of \"ExchangeRate\" by \"ExchangeRateId\" with value {ExchangeRateId} was denied to the user with Login ID {_LoginId}", exchangeRateId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.exchange_rates WHERE exchange_rate_id > @0 ORDER BY exchange_rate_id LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Core.ExchangeRate>(this._Catalog, sql, exchangeRateId).FirstOrDefault();
        }


        /// <summary>
        /// Gets the last record of the table "core.exchange_rates". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "ExchangeRate" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.Core.ExchangeRate GetLast()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the last record of entity \"ExchangeRate\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.exchange_rates ORDER BY exchange_rate_id DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Core.ExchangeRate>(this._Catalog, sql).FirstOrDefault();
        }
