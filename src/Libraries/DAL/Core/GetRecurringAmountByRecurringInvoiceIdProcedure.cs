...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class GetRecurringAmountByRecurringInvoiceIdProcedure : DbAccess, IGetRecurringAmountByRecurringInvoiceIdRepository
    {
        ...
        public decimal Execute()
        {
            ...
            string query = "SELECT * FROM core.get_recurring_amount_by_recurring_invoice_id(@RecurringInvoiceId);";

            query = query.ReplaceWholeWord("@RecurringInvoiceId", "@0::integer");


            List<object> parameters = new List<object>();
            parameters.Add(this.RecurringInvoiceId);
