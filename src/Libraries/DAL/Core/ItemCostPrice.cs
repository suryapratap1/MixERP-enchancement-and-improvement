...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    /// </summary>
    public class ItemCostPrice : DbAccess, IItemCostPriceRepository
    {
        /// <summary>
        ...
        /// </summary>
        public override string _ObjectName => "item_cost_prices";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "core.item_cost_prices" to return all instances of the "ItemCostPrice" class. 
        /// </summary>
        ...
        /// <summary>
        /// Executes a select query on the table "core.item_cost_prices" to return all instances of the "ItemCostPrice" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "ItemCostPrice" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        public MixERP.Net.Entities.Core.ItemCostPrice Get(long itemCostPriceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"ItemCostPrice\" filtered by \"ItemCostPriceId\" with value {ItemCostPriceId} was denied to the user with Login ID {_LoginId}", itemCostPriceId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ItemCostPrice> Get(long[] itemCostPriceIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. itemCostPriceIds: {itemCostPriceIds}.", this._LoginId, itemCostPriceIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.ItemCostPrice>(this._Catalog, sql, itemCostPriceIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public object AddOrEdit(dynamic itemCostPrice, List<EntityParser.CustomField> customFields)
        {
            ...
            object primaryKeyValue = itemCostPrice.item_cost_price_id;
        ...
        public object Add(dynamic itemCostPrice)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. {ItemCostPrice}", this._LoginId, itemCostPrice);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public List<object> BulkImport(List<ExpandoObject> itemCostPrices)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. {itemCostPrices}", this._LoginId, itemCostPrices);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Update(dynamic itemCostPrice, long itemCostPriceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"ItemCostPrice\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {ItemCostPrice}", itemCostPriceId, this._LoginId, itemCostPrice);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Delete(long itemCostPriceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"ItemCostPrice\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", itemCostPriceId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.item_cost_prices WHERE item_cost_price_id=@0;";
            Factory.NonQuery(this._Catalog, sql, itemCostPriceId);
        }

        /// <summary>
        /// Performs a select statement on table "core.item_cost_prices" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ItemCostPrice> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.item_cost_prices ORDER BY item_cost_price_id LIMIT 10 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.ItemCostPrice>(this._Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ItemCostPrice> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.ItemCostPrice>(this._Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.item_cost_prices" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ItemCostPrice> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ItemCostPrice> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"ItemCostPrice\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
