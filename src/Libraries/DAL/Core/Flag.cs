...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class Flag : DbAccess, IFlagRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "flags";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Flag\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Flag> GetAll()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"Flag\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.flags ORDER BY flag_id;";
            return Factory.Get<MixERP.Net.Entities.Core.Flag>(this._Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.Core.Flag Get(long flagId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"Flag\" filtered by \"FlagId\" with value {FlagId} was denied to the user with Login ID {_LoginId}", flagId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Executes a select query on the table "core.flags" with a where filter on the column "flag_id" to return a multiple instances of the "Flag" class. 
        /// </summary>
        /// <param name="flagIds">Array of column "flag_id" parameter used on where filter.</param>
        /// <returns>Returns a non-live, non-mapped collection of "Flag" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public IEnumerable<MixERP.Net.Entities.Core.Flag> Get(long[] flagIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"Flag\" was denied to the user with Login ID {LoginId}. flagIds: {flagIds}.", this._LoginId, flagIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.Flag>(this._Catalog, sql, flagIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"Flag\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"Flag\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public object AddOrEdit(dynamic flag, List<EntityParser.CustomField> customFields)
        {
            ...
            {
                primaryKeyValue = this.Add(flag);
            }

            string sql = "DELETE FROM core.custom_fields WHERE custom_field_setup_id IN(" +
                         "SELECT custom_field_setup_id " +
                         "FROM core.custom_field_setup " +
                         "WHERE form_name=core.get_custom_field_form_name('core.flags')" +
                         ");";
            ...
            foreach (var field in customFields)
            {
                sql = "INSERT INTO core.custom_fields(custom_field_setup_id, resource_id, value) " +
                      "SELECT core.get_custom_field_setup_id_by_table_name('core.flags', @0::character varying(100)), " +
                      "@1, @2;";
        ...
        public object Add(dynamic flag)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"Flag\" was denied to the user with Login ID {LoginId}. {Flag}", this._LoginId, flag);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this._Catalog, flag, "core.flags", "flag_id");
        }

        /// <summary>
        /// Inserts or updates multiple instances of Flag class on the database table "core.flags";
        /// </summary>
        ...
        public List<object> BulkImport(List<ExpandoObject> flags)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"Flag\" was denied to the user with Login ID {LoginId}. {flags}", this._LoginId, flags);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                ...
                using (Database db = new Database(Factory.GetConnectionString(this._Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (dynamic flag in flags)
                        {
                            ...
                            {
                                result.Add(db.Insert("core.flags", "flag_id", flag));
                            }
                        }
        ...
        public void Update(dynamic flag, long flagId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"Flag\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {Flag}", flagId, this._LoginId, flag);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this._Catalog, flag, flagId, "core.flags", "flag_id");
        }
        ...
        public void Delete(long flagId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"Flag\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", flagId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.flags WHERE flag_id=@0;";
            Factory.NonQuery(this._Catalog, sql, flagId);
        }

        /// <summary>
        /// Performs a select statement on table "core.flags" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Flag> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"Flag\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.flags ORDER BY flag_id LIMIT 10 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.Flag>(this._Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Flag> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"Flag\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered count on table "core.flags".
        /// </summary>
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Flag\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Sql sql = Sql.Builder.Append("SELECT COUNT(*) FROM core.flags WHERE 1 = 1");
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.flags" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Flag> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"Flag\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Flag\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Flag> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"Flag\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
