...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    /// </summary>
    public class CashFlowSetup : DbAccess, ICashFlowSetupRepository
    {
        /// <summary>
        ...
        /// </summary>
        public override string _ObjectName => "cash_flow_setup";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}", this._LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "core.cash_flow_setup" to return all instances of the "CashFlowSetup" class. 
        /// </summary>
        ...
        /// <summary>
        /// Executes a select query on the table "core.cash_flow_setup" to return all instances of the "CashFlowSetup" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "CashFlowSetup" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        public MixERP.Net.Entities.Core.CashFlowSetup Get(int cashFlowSetupId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"CashFlowSetup\" filtered by \"CashFlowSetupId\" with value {CashFlowSetupId} was denied to the user with Login ID {_LoginId}", cashFlowSetupId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowSetup> Get(int[] cashFlowSetupIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. cashFlowSetupIds: {cashFlowSetupIds}.", this._LoginId, cashFlowSetupIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.CashFlowSetup>(this._Catalog, sql, cashFlowSetupIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public object Add(dynamic cashFlowSetup)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. {CashFlowSetup}", this._LoginId, cashFlowSetup);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public List<object> BulkImport(List<ExpandoObject> cashFlowSetups)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. {cashFlowSetups}", this._LoginId, cashFlowSetups);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Update(dynamic cashFlowSetup, int cashFlowSetupId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"CashFlowSetup\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {CashFlowSetup}", cashFlowSetupId, this._LoginId, cashFlowSetup);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Delete(int cashFlowSetupId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"CashFlowSetup\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", cashFlowSetupId, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.cash_flow_setup WHERE cash_flow_setup_id=@0;";
            Factory.NonQuery(this._Catalog, sql, cashFlowSetupId);
        }

        /// <summary>
        /// Performs a select statement on table "core.cash_flow_setup" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowSetup> GetPaginatedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.cash_flow_setup ORDER BY cash_flow_setup_id LIMIT 10 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.CashFlowSetup>(this._Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowSetup> GetPaginatedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}.", pageNumber, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.CashFlowSetup>(this._Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.cash_flow_setup" producing a paginated result of 10.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowSetup> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this._LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowSetup> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"CashFlowSetup\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this._LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
