...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class AppendIfNotNullProcedure : DbAccess, IAppendIfNotNullRepository
    {
        ...
        public string Execute()
        {
            ...
            string query = "SELECT * FROM core.append_if_not_null(@Source, @ToAppend);";

            query = query.ReplaceWholeWord("@Source", "@0::text");
            query = query.ReplaceWholeWord("@ToAppend", "@1::text");


            List<object> parameters = new List<object>();
            parameters.Add(this.Source);
            parameters.Add(this.ToAppend);
