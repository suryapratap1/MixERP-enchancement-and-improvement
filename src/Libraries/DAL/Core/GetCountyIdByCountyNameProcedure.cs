...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class GetCountyIdByCountyNameProcedure : DbAccess, IGetCountyIdByCountyNameRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "get_county_id_by_county_name";
        /// <summary>
        ...
        public int Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"GetCountyIdByCountyNameProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            string query = "SELECT * FROM core.get_county_id_by_county_name(@PgArg0);";
