...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class GetShippingAddressIdByShippingAddressCodeProcedure : DbAccess, IGetShippingAddressIdByShippingAddressCodeRepository
    {
        ...
        /// </summary>
        public override string _ObjectName => "get_shipping_address_id_by_shipping_address_code";
        /// <summary>
        ...
        public int Execute()
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the function \"GetShippingAddressIdByShippingAddressCodeProcedure\" was denied to the user with Login ID {LoginId}.", this._LoginId);
