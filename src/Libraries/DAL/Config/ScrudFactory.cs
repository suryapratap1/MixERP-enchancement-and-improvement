...
namespace MixERP.Net.Schemas.Config.Data
{
    ...
    /// </summary>
    public class ScrudFactory : DbAccess, IScrudFactoryRepository
    {
        /// <summary>
        ...
        /// <summary>
        /// Executes a select query on the table "config.scrud_factory" to return all instances of the "ScrudFactory" class. 
        /// </summary>
        ...
        /// <summary>
        /// Executes a select query on the table "config.scrud_factory" to return all instances of the "ScrudFactory" class to export. 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instances of "ScrudFactory" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        ...
        /// <summary>
        /// Gets the first record of the table "config.scrud_factory". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "ScrudFactory" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.Config.ScrudFactory GetFirst()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the first record of entity \"ScrudFactory\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.scrud_factory ORDER BY key LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Config.ScrudFactory>(this._Catalog, sql).FirstOrDefault();
        }

        /// <summary>
        /// Gets the previous record of the table "config.scrud_factory" sorted by key.
        /// </summary>
        ...
        public MixERP.Net.Entities.Config.ScrudFactory GetPrevious(string key)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the previous entity of \"ScrudFactory\" by \"Key\" with value {Key} was denied to the user with Login ID {_LoginId}", key, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.scrud_factory WHERE key < @0 ORDER BY key DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Config.ScrudFactory>(this._Catalog, sql, key).FirstOrDefault();
        }

        /// <summary>
        /// Gets the next record of the table "config.scrud_factory" sorted by key.
        /// </summary>
        /// <param name="key">The column "key" parameter used to find the next record.</param>
        /// <returns>Returns a non-live, non-mapped instance of "ScrudFactory" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.Config.ScrudFactory GetNext(string key)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the next entity of \"ScrudFactory\" by \"Key\" with value {Key} was denied to the user with Login ID {_LoginId}", key, this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.scrud_factory WHERE key > @0 ORDER BY key LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Config.ScrudFactory>(this._Catalog, sql, key).FirstOrDefault();
        }


        /// <summary>
        /// Gets the last record of the table "config.scrud_factory". 
        /// </summary>
        /// <returns>Returns a non-live, non-mapped instance of "ScrudFactory" class mapped to the database row.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public MixERP.Net.Entities.Config.ScrudFactory GetLast()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get the last record of entity \"ScrudFactory\" was denied to the user with Login ID {_LoginId}", this._LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.scrud_factory ORDER BY key DESC LIMIT 1;";
            return Factory.Get<MixERP.Net.Entities.Config.ScrudFactory>(this._Catalog, sql).FirstOrDefault();
        }
